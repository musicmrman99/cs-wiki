...
After making any desired changes to the files in the working tree, the changes must be added to the **index** before they can be committed. The index stores what the next commit would be if the user committed right now - it acts like a 'staging area' before the changes it contains are committed. Once the index contains a single stable and coherent set of changes, the user can commit those changes.
...
[^1]: Non-bare clones. Bare clones only contain the history, with no working tree. Bare repositories are intended to be used for integrating a team's changes, rather than as a clone for making changes to the tracked files in the repository. Version control hubs usually use bare clones for the repositories they manage to save on storage costs.
